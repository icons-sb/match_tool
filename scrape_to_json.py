...
scrape_to_json.py
─────────────────
Scrapa il portale EU Funding & Tenders con la Search API REST (bypassando
il shadow DOM del portale) e produce calls.json direttamente.
Usa Playwright solo per l'arricchimento dei dettagli delle singole call.

Uso:
    python scrape_to_json.py              # scrive calls.json nella cartella corrente
    python scrape_to_json.py --out /path  # percorso custom

NOTA v2: il portale ha migrato i contenuti in Web Components (shadow DOM),
rendendo inaccessibili i link via DOM normale. La lista viene ora recuperata
direttamente dalla Search API REST intercettata:
...
# Search API REST — bypassiamo il portale Angular/shadow-DOM
SEARCH_API_BASE = "https://api.tech.ec.europa.eu/search-api/prod/rest/search"
SEARCH_API_KEY  = "SEDIA"

# Filtri: status 31094501=OPEN, 31094502=FORTHCOMING; periodo 2021-2027
SEARCH_QUERY_PARAMS = {
    "apiKey":          SEARCH_API_KEY,
    "text":            "*",
    "pageSize":        str(PAGE_SIZE),
    "pageNumber":      "1",
    # Filtri avanzati come query string addizionale
    "query": (
        "type=1&"
...
# URL base per costruire i link alle call
...
def _result_to_url(result: dict) -> str:
    """
    Ricava l'URL della call dal risultato della Search API.
    Prova prima metadata.identifier, poi il campo 'url', poi costruisce da callIdentifier.
    """
    meta = result.get("metadata", {}) or {}

    # 1. URL diretto nei metadati
    for key in ("callIdentifier", "identifier", "topicIdentifier"):
        val = meta.get(key)
        if isinstance(val, list):
            val = val[0] if val else None
        if val:
            val = str(val).strip()
            # Se sembra già un URL completo
            if val.startswith("http"):
                return val
            # Costruiamo l'URL dal topic/call identifier
            # Prova prima come topic
            if any(c in val.upper() for c in ["-", "_"]):
                return f"{TOPIC_BASE}/{val}"

    # 2. Campo url esplicito
    url = result.get("url") or meta.get("url")
    if url:
        return str(url).strip()

    # 3. Ricava da callIdentifier nel campo 'id' del risultato
    rid = result.get("id", "")
    if rid:
...
def _result_to_row(result: dict) -> dict | None:
    """Converte un risultato della Search API in una riga dati."""
    meta = result.get("metadata", {}) or {}
    ...
    # Programme
    prog_id = _first(meta, "frameworkProgramme", "programme", "programmePeriod")
    ...
    call_id = _first(meta, "callIdentifier", "identifier", "topicIdentifier")

    # Cluster
    cluster_raw = ""
    for src in [call_id, url]:
    ...
    opening_raw  = _date("startDate") or _date("openingDate")
    deadline_raw = _date("deadlineDate") or _date("nextDeadline") or _date("deadline")

    # Budget
    budget_raw = 0
    for key in ("budgetOverviewTotal","totalBudget","budget","indicativeBudget","availableBudget","estimatedTotalContribution"):
        raw = meta.get(key)
...
def fetch_all_calls_via_api() -> list:
    """
    Scarica tutte le call dalla Search API REST con paginazione.
    Restituisce lista di righe già parzialmente compilate.
    """
    print("═══ Passo 1: raccolta lista call via Search API REST ═══", flush=True)

    # Prima richiesta per sapere il totale
    first_url  = _build_search_url(1)
    print(f"  → {first_url[:100]}…", flush=True)
    first_data = _fetch_json(first_url)

    total = first_data.get("totalResults") or first_data.get("total") or 0
    if not total:
        # Prova a contare i risultati disponibili
        results = first_data.get("results") or []
        total   = len(results)
    ...
    def _process_page(data: dict):
        for result in data.get("results", []):
            row = _result_to_row(result)
            if not row:
                continue
            uid = row["url"] or row["call_id"] or row["name"]
            if uid and uid not in seen_ids:
                seen_ids.add(uid)
                rows.append(row)

    _process_page(first_data)
    print(f"  [p1/{max_pages}] +{len(rows)} call", flush=True)
...
def extract_budget_per_project_dom(page, topic_id):
    """Espande la tabella Topic conditions e preleva il budget semantico."""
...
def _enrich_one(page, row: dict) -> bool:
    """
    Apre la pagina di dettaglio e integra campi mancanti.
    Intercetta le risposte XHR della Search API per dati strutturati,
    poi legge il testo del body per keyword e budget fallback.
    """
    url      = row["url"]
            ...
            try:
                ...
                for item in body.get("results", [body]):
                    ...
                    if not _c.get("budget"):
                        for key in ("budgetOverviewTotal","totalBudget","budget","budgetTopicActions",
                                    "indicativeBudget","availableBudget","estimatedTotalContribution"):
                            raw = meta.get(key)
                            if isinstance(raw, list): raw = raw[0] if raw else None
                            if raw is not None:
                                val = parse_budget(str(raw))
...
def enrich(ctx, rows: list):
    """Arricchisce le call che mancano di dati essenziali aprendo le pagine di dettaglio."""
...
def main(out_path: Path):
    # ── Passo 1: raccolta lista via API REST (no browser) ─────────────────────
    ...
    # ── Passo 2: arricchimento via Playwright ─────────────────────────────────
