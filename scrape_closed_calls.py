...
closed_calls.json — chiamate con status CLOSED (31094503).

Uso:
    python scrape_closed_calls.py              # scrive closed_calls.json nella cartella corrente
    python scrape_closed_calls.py --out /path  # percorso custom
    python scrape_closed_calls.py --max-pages 5  # limita le pagine (per test)
"""
...
# ── Tabelle di classificazione (identiche allo scraper open) ──────────────────
...
def main(out_path: Path, max_pages: int = None):
    rows      = []
    seen_urls = set()
    ...
    with sync_playwright() as p:
        ...
        print(f"✅ Totale chiamate chiuse: {total} | pagine: {real_max_pages} (elaborerò: {effective_max})")
    ...
    for c in calls:
        ...
        tc[k] = tc.get(k, 0) + 1
    print(f"\nClassificazione ({len(calls)} call chiuse totali):")
    ...
    # Compact lookup: url-slug → {thematic, multi_thematic, action, call_id, name}
    autofill_index = {}
    for c in calls:
    ...
    print(f"\n✅ Scritto {out_path} con {len(calls)} call chiuse")

    # ── Save slim autofill index separately for fast frontend loading ─────────
    index_path = out_path.parent / "closed_calls_autofill_index.json"
    index_payload = {
        "generated": generated,
        "index":     autofill_index,
    }
    index_path.write_text(
        json.dumps(index_payload, ensure_ascii=False, separators=(",",":")),
        encoding="utf-8",
    )
    print(f"✅ Index autofill separato: {index_path} ({len(autofill_index)} entries)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapa le call CHIUSE dal portale EU F&T")
    parser.add_argument("--out",       default="closed_calls.json", help="Percorso output JSON")
    parser.add_argument("--max-pages", type=int, default=None,      help="Numero massimo pagine (per test)")
    args = parser.parse_args()
    main(Path(args.out), args.max_pages)
